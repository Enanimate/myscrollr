...
Only desktop releases have CI (`.github/workflows/desktop-release.yml`). Triggers on push to `main` when `desktop/` changes. Builds for Linux/macOS/Windows via `tauri-action`. No CI for website, Go APIs, or Rust services.
...
- Edition 2021 (not 2024). `lib.rs` is the main entry point (~1300 lines).
...
- Two windows: `ticker` (always-on-top, no decorations) and `main` (centered, min 720x480). Close hides instead of destroying.
