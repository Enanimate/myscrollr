...
pub async fn log_writer_task(mut receiver: mpsc::Receiver<LogMessage>, log_file_path: String) {
    ...
    let mut rss_log = match File::create(format!("{}/rss.log", log_file_path)) {
        Ok(f) => f,
        Err(e) => {
            eprintln!("Fatal: Could not create log file at {:?}: {}", "rss.log", e);
            return;
        }
    };
    ...
    while let Some(msg) = receiver.recv().await {
        ...
        if let Err(e) = rss_log.write_all(msg.as_bytes()) {
