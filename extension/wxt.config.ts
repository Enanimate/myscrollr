 ...
 * extension directory (e.g. channels/star/extension/) to this project's
 * node_modules.
 *
...
export default defineConfig({
  ...
  alias: {
    "@scrollr": resolve("..", "channels"),
  },
  manifest: {
    name: "Scrollr",
    description: "Real-time financial and sports data feed for your browser",
    permissions: ["storage", "identity", "alarms"],
    // PROD: relentnet.dev values: api.myscrollr.relentnet.dev, auth.myscrollr.relentnet.dev
    host_permissions: [
  ...
  vite: () => ({
    plugins: [tailwindcss(), resolveExternalChannels()],
    css: {
      postcss: {
        plugins: [
          remToPixel({
